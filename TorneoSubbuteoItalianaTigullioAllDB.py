...
def inject_parent_head_assets(is_campionato: bool):
    ...
    manifest_data = {
        ...
        "short_name": "Tigullio",
        "description": "Suite mobile per tornei Subbuteo Tigullio",
        "start_url": "/",
        "scope": "/",
        "display": "standalone",
        ...
        "icons": [
            {
                "src": "/app/static/logo_tigullio.jpg",
                "sizes": "192x192",
                "type": "image/jpeg",
    ...
    if is_campionato:
        manifest_data["shortcuts"] = [
            {
                "name": "Campionato Tigullio 26/27",
                "short_name": "Campionato",
                "description": "Apri direttamente il Campionato Tigullio 26/27",
                "url": "/?torneo=CampionatoTigullio_26_27",
            }
        ]
    components.html(
        f"""
...
HOME_URL = "https://torneo-subbuteo-tigullio-ita-all-db.streamlit.app/"
...
def redirect_to_final_phase(torneo_nome):
    ...
    redirect_url = f"https://torneo-subbuteo-ff-tigullio-ita-all-db.streamlit.app/?torneo={urllib.parse.quote(torneo_nome)}"
    st.markdown(
        f"""
        <script>
...
def navigation_buttons(label: str, value_key: str, min_val: int, max_val: int, key_prefix: str = ""):
    ...
    st.html("""
        <style>
        .st-key-tigullio_italiana_navigation [data-testid="stHorizontalBlock"] {
            display: grid !important;
            grid-template-columns: 48px minmax(0, 1fr) 48px;
            align-items: center !important;
            gap: 8px !important;
        }
        .st-key-tigullio_italiana_navigation [data-testid="stHorizontalBlock"] > div {
            width: 100% !important;
            min-width: 0 !important;
            max-width: none !important;
            margin: 0 !important;
        }
        .st-key-tigullio_italiana_navigation [data-testid="stColumn"] > [data-testid="stVerticalBlock"] {
            height: auto !important;
            min-height: 44px;
            justify-content: center !important;
        }
        .st-key-tigullio_italiana_navigation button {
        ...
        .st-key-tigullio_italiana_navigation button:not(:disabled) {
            background: linear-gradient(to right, var(--color-primary-mid), var(--color-primary-light));
            color: white;
        }
        .st-key-tigullio_italiana_navigation .tigullio-day-label {
            text-align: center;
            font-size: 16px;
            font-weight: 900;
            letter-spacing: 0;
        ...
        .st-key-tigullio_italiana_navigation [data-testid="stMarkdownContainer"] {
            margin-bottom: 0 !important;
        }
        </style>
    """)

    with st.container(key="tigullio_italiana_navigation"):
        ...
        with col_label:
            st.markdown(
                f"<div class='tigullio-day-label' style='text-align:center;font-weight:900;'>"
...
def applica_stile_riga_calendario(selector=".st-key-tigullio_italiana_compact"):
    # Un vero container Streamlit include i widget; due markdown <div> non li racchiudono.
    st.html("""
        <style>
        .st-key-tigullio_italiana_compact [data-testid="stHorizontalBlock"] {
            display: grid !important;
        ...
        .st-key-tigullio_italiana_compact [data-testid="stHorizontalBlock"] > div {
            width: 100% !important;
            min-width: 0 !important;
            max-width: none !important;
            margin: 0 !important;
        }
        .st-key-tigullio_italiana_compact .tigullio-team-name {
            font-size: 16px;
            font-weight: 800;
            letter-spacing: 0;
            line-height: 1.25;
        ...
        .st-key-tigullio_italiana_compact [data-testid="stColumn"] > [data-testid="stVerticalBlock"] {
            justify-content: center !important;
        }
        .st-key-tigullio_italiana_compact [data-testid="stElementContainer"],
        .st-key-tigullio_italiana_compact [data-testid="stCheckbox"] {
            width: 100% !important;
        }
        .st-key-tigullio_italiana_compact .tigullio-team-name.away {
            text-align: right;
        }
        .st-key-tigullio_italiana_compact [data-testid="stMarkdownContainer"] {
            margin-bottom: 0 !important;
        }
        .st-key-tigullio_italiana_compact [data-testid="stNumberInput"],
        .st-key-tigullio_italiana_compact [data-baseweb="input"],
        .st-key-tigullio_italiana_compact [data-baseweb="base-input"] {
            width: 100% !important;
            min-width: 0 !important;
            max-width: none !important;
            margin: 0 !important;
        ...
        .st-key-tigullio_italiana_compact [data-testid="stNumberInput"] input {
            width: 100% !important;
            min-width: 0 !important;
        ...
        .st-key-tigullio_italiana_compact [data-testid="stNumberInputContainer"] {
            height: 44px !important;
        }
        .st-key-tigullio_italiana_compact [data-testid="stNumberInput"] button,
        .st-key-tigullio_italiana_compact input::-webkit-inner-spin-button,
        .st-key-tigullio_italiana_compact input::-webkit-outer-spin-button {
            display: none !important;
        }
        .st-key-tigullio_italiana_compact [data-testid="stCheckbox"] label {
    ...
    """.replace(".st-key-tigullio_italiana_compact", selector))
...
def mostra_calendario_premium(df, girone_sel, giornata_sel, modalita_visualizzazione):
    ...
    applica_stile_riga_calendario('[class*="st-key-tigullio_italiana_premium_"]')
    ...
    for idx, row in df_giornata.iterrows():
        ...
        with st.container(border=True, key=f"tigullio_italiana_premium_{idx}"):
            # Header della card
            ...
            with c1:
                st.markdown(f"<div style='text-align:right;' class='tigullio-team-name home'>🏠 {escape(str(label_c))}</div>", unsafe_allow_html=True)
            ...
            with c4:
                st.markdown(f"<div style='text-align:left;' class='tigullio-team-name away'>{escape(str(label_o))} 🛫</div>", unsafe_allow_html=True)
...
def mostra_calendario_compact(df, girone_sel, giornata_sel, modalita_visualizzazione):
    """Una riga per incontro, con widget nativi e stili limitati all'Italiana Tigullio."""
    df_giornata = df[(df['Girone'] == girone_sel) & (df['Giornata'] == giornata_sel)].copy()
    if df_giornata.empty:
        return
    ...
    with st.container(key="tigullio_italiana_compact"):
        for idx, row in df_giornata.iterrows():
            match_validated = bool(draft_value(f"valida_{girone_sel}_{giornata_sel}_{row['Casa']}_{row['Ospite']}", bool(row['Valida'])))
            casa, gio_c = parse_team_player(row['Casa'])
            osp, gio_o = parse_team_player(row['Ospite'])
            ...
            with casa_col:
                st.markdown(
                    f"<div class='tigullio-team-name home'>{escape(str(label_c))}</div>",
                    unsafe_allow_html=True
                )
            with gc_col:
            ...
            with osp_col:
                st.markdown(
                    f"<div class='tigullio-team-name away'>{escape(str(label_o))}</div>",
                    unsafe_allow_html=True
                )
            with valida_col:
...
class GazzettaPDF(FPDF):
    ...
    def header(self):
        ...
        # 🛡️ Logo "Tigullio" a sinistra (se disponibile)
...
def render_app_title():
    ...
    st.markdown(f"""
        <style>
        div[data-testid="stElementContainer"]:has(div.st-key-tigullio_banner_audio_button),
        div.element-container:has(div.st-key-tigullio_banner_audio_button) {{
            display: flex !important;
            justify-content: center !important;
        ...
        div.st-key-tigullio_banner_audio_button {{
            display: flex !important;
            justify-content: center !important;
            position: relative;
            width: 100% !important;
        ...
        div.st-key-tigullio_banner_audio_button button {{
            position: relative;
        ...
        div.st-key-tigullio_banner_audio_button button:hover {{
            transform: translateY(-1px) scale(1.05);
            filter: brightness(1.06);
        }}
        div.st-key-tigullio_banner_audio_button button:active {{
            transform: translateY(0) scale(0.98);
        }}
        div.st-key-tigullio_banner_audio_button label {{
            display: flex !important;
            align-items: center !important;
            gap: 4px !important;
        ...
        div.st-key-tigullio_banner_audio_button label p {{
        ...
        div.st-key-tigullio_banner_audio_button [data-baseweb="checkbox"] {{
    ...
    if st.button(audio_label, key="tigullio_banner_audio_button", help=audio_help):
...
def main():
    ...
    render_app_title()
    inject_css()
    inject_parent_head_assets(torneo_manifest == "CampionatoTigullio_26_27")
    enable_session_keepalive()
    ...
    players_collection = init_mongo_connection(st.secrets["MONGO_URI"], "giocatori_subbuteo", "tigullio_players", show_ok=False)
    tournaments_collection = init_mongo_connection(st.secrets["MONGO_URI_TOURNEMENTS"], "TorneiSubbuteo", "Tigullio", show_ok=False)
