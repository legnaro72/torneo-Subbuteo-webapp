...
# Rileva se siamo nel club Superba per mostrare il Palmares (solo per Superba)
IS_SUPERBA = "superba" in collection_players.name.lower()
...
class GazzettaClubPDF(FPDF):
    """PDF con stile 'La Gazzetta della Superba' per la composizione del club."""
    ...
    def header(self):
        ...
        self.cell(0, 10, "IL GAZZETTINO DELLA SUPERBA", border=0, new_x="LMARGIN", new_y="NEXT", align='L')
        # Sottotitolo
        self.set_x(start_x)
        self.set_font("helvetica", 'I', 11)
        self.set_text_color(220, 225, 235)
        data_stampa = datetime.now().strftime("%d/%m/%Y alle %H:%M")
        self.cell(0, 6, f"Composizione Club Superba | Aggiornato il {data_stampa}", border=0, new_x="LMARGIN", new_y="NEXT", align='L')
        self.ln(12)
    ...
    def footer(self):
        ...
        self.set_text_color(255, 255, 255)
        self.cell(0, 10, f'Pagina {self.page_no()} - La Gazzetta della Superba - Gestionale Club Subbuteo', 0, 0, 'C')
...
def genera_pdf_club(df_giocatori, df_tornei_ita, df_tornei_svizzeri, is_superba=True):
    ...
    if is_superba and not df_giocatori.empty:
        pdf.add_page()
        pdf.set_font("helvetica", 'B', 18)
        pdf.set_fill_color(230, 235, 245)
        pdf.set_text_color(26, 54, 93)
...
if st.session_state.edit_index is None and st.session_state.confirm_delete["type"] is None:
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("📰 La Gazzetta della Superba")
    ...
    if 'pdf_club_pronto' in st.session_state and st.session_state['pdf_club_pronto'] is not None:
        st.sidebar.download_button(
            label="📥 Scarica PDF Club Superba",
            data=st.session_state['pdf_club_pronto'],
            file_name=f"Gazzetta_Club_Superba_{datetime.now().strftime('%d%m%Y')}.pdf",
