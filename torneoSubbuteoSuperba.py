...
st.set_page_config(page_title="⚽️ Gestione Torneo Superba a Gironi by Legnaro72", layout="wide")
...
def mostra_classifica_stilizzata(df_classifica, girone_sel):
    ...
    if df_classifica is None or df_classifica.empty:
        st.info("ℹ️ Nessuna partita validata: la classifica sarà disponibile dopo l'inserimento e validazione dei risultati.")
        return
...
def main():
    st.title("⚽️ Gestione Torneo Superba a Gironi by Legnaro72")
    ...
    if scelta == "Nuovo torneo":
        num_gironi = st.number_input("Numero di gironi 🏟️", 1, 8, value=2)
        tipo_calendario = st.selectbox("Tipo calendario 🗓️", ["Solo andata", "Andata e ritorno"])
        n_giocatori = st.number_input("Numero giocatori 🎲", 4, 32, value=8)

        st.markdown("### Amici del Club 🤝")
        amici = df_master['Giocatore'].tolist()
        all_seleziona = st.checkbox("Seleziona tutti gli amici ✔️", key="all_amici")
    ...
    if 'giocatori_scelti' in st.session_state and scelta == "Nuovo torneo":
        st.markdown("### ✍️ Modifica e genera calendario")

        if 'df_torneo' not in st.session_state:
            df_torneo = genera_calendario(st.session_state['giocatori_scelti'], st.session_state['num_gironi'], st.session_state['tipo_calendario'])
            st.session_state['df_torneo'] = df_torneo
        else:
            df_torneo = st.session_state['df_torneo']

        gironi = df_torneo['Girone'].unique()
        girone_sel = st.selectbox("Seleziona girone per gestione calendario 📅", gironi)

        giornate = sorted(df_torneo[df_torneo['Girone'] == girone_sel]['Giornata'].unique())
        giornata_sel = st.selectbox("Seleziona giornata", giornate)

        mostra_calendario_giornata(df_torneo, girone_sel, giornata_sel)

        df_classifica = aggiorna_classifica(df_torneo)
        ...
        csv = df_torneo.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Scarica CSV Torneo", data=csv, file_name="torneo_gironi.csv", mime="text/csv")

        if df_classifica is not None and not df_classifica.empty:
            pdf_bytes = esporta_pdf(df_torneo, df_classifica)
            st.download_button("📥 Scarica PDF Calendario + Classifiche", data=pdf_bytes, file_name="torneo_superba.pdf", mime="application/pdf")
