...
app = FastAPI(title='Superba', docs_url='/api/docs', openapi_url='/api/openapi.json')
COOKIE = 'superba_portal_session'
DESTINATIONS = {
    'finali': 'https://torneo-subbuteo-ff-superba-ita-all-db.streamlit.app/',
    'svizzero': 'https://torneo-subbuteo-superba-new-version-svizzero-alldb.streamlit.app/',
    'club': 'https://edit-superba-club-all-db-new.streamlit.app/',
    'italiana-classica': 'https://torneo-subbuteo-superba-ita-all-db.streamlit.app/',
}
...
async def guard(request: Request, call_next):
    if request.method not in ('GET', 'HEAD', 'OPTIONS'):
        expected = os.getenv('SUPERBA_APP_ORIGIN', str(request.base_url).rstrip('/'))
        origin = request.headers.get('origin')
        if request.headers.get('x-superba-request') != '1' or (origin and origin != expected):
...
def writer(user=Depends(current_user), store: Store = Depends(store_dep)):
    ...
    if not store.demo and os.getenv('SUPERBA_WRITE_ENABLED', '').lower() != 'true':
...
def establish(response, store, player=None, *, remember=False, verified=False):
    ...
    secure = bool(os.getenv('VERCEL')) or os.getenv('SUPERBA_APP_ORIGIN', '').startswith('https://')
...
@app.get('/api/config')
def config():
    demo = os.getenv('SUPERBA_DEMO', '').lower() == 'true' and not os.getenv('VERCEL')
    return {'demo': demo, 'writes_enabled': demo or os.getenv('SUPERBA_WRITE_ENABLED', '').lower() == 'true'}
...
def handoff(destination: str, user=Depends(current_user), store: Store = Depends(store_dep)):
    ...
    store.handoffs.insert_one({'token_hash': hash_token(token), 'user_id': user['id'], 'username': user['username'],
                               'role': user['role'], 'collection': 'superba_players', 'created_at': now,
...
def create(data: CreateTournament, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    if existing:
        if existing.get('_superba_create_hash') != payload_hash:
    ...
    doc = {'_id': new_id, 'nome_torneo': data.name, 'calendario': calendar.to_dict('records'), 'data_creazione': datetime.utcnow(),
           'data_modifica': datetime.utcnow(), '_superba_revision': 0, '_superba_create_hash': payload_hash,
           '_superba_return_matches': data.return_matches}
    if data.badges:
        doc['_superba_badges'] = {key: badge.model_dump(exclude_none=True) for key, badge in data.badges.items()}
    if data.participants is not None:
        doc['_superba_participants'] = [p.model_dump() for p in data.participants]
    try:
        store.tournaments.insert_one(doc)
    except DuplicateKeyError:
        existing = store.tournaments.find_one({'_id': new_id})
        if not existing or existing.get('_superba_create_hash') != payload_hash:
...
def withdrawal(tournament_id: str, data: Withdrawal, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    retired = set(doc.get('_superba_withdrawals', [])) | set(data.teams)
    rows = [dict(row) for row in doc['calendario']]
    for row in rows:
    ...
    return view(save(store, doc, data.version, {'calendario': rows, '_superba_withdrawals': sorted(retired)}))
...
def complete(tournament_id: str, data: Complete, user=Depends(writer), store: Store = Depends(store_dep)):
    doc = load(store, tournament_id)
    require_tournament_write(user, doc['nome_torneo'])
    if doc.get('_superba_source_id') or doc['nome_torneo'].startswith('completato_'):
    ...
    if doc.get('_superba_closed'):
        if doc.get('_superba_closed_from') != data.version and rendered['version'] != data.version:
            raise HTTPException(409, 'Ricarica il torneo prima di concluderlo.')
    else:
        doc = save(store, doc, data.version, {'_superba_closed': True, '_superba_closed_from': data.version})
    ...
    if not store.tournaments.find_one({'nome_torneo': completed_name}):
        archive = {**doc, '_id': archive_id, 'nome_torneo': completed_name, '_superba_source_id': tournament_id}
    ...
    for group in groups:
        ...
        store.players.update_one({'_id': player['_id'], '_superba_award_ids': {'$ne': award_id}, list_field: {'$ne': name}},
                                 {'$addToSet': {list_field: name, '_superba_award_ids': award_id}, '$inc': {count_field: 1}})
...
def export_csv(tournament_id: str, user=Depends(current_user), store: Store = Depends(store_dep)):
    ...
    return Response(output.getvalue().encode('utf-8-sig'), media_type='text/csv', headers={'Content-Disposition': 'attachment; filename="calendario-superba.csv"'})
...
def export_pdf(tournament_id: str, user=Depends(current_user), store: Store = Depends(store_dep)):
    data = view(load(store, tournament_id))
    return Response(render_tournament_pdf(data), media_type='application/pdf', headers={'Content-Disposition': 'attachment; filename="gazzettino-superba.pdf"'})
...
def save_badges(kind: str, tournament_id: str, data: SaveBadges, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    return render(save(store, doc, data.version, {'_superba_badges': {key: badge.model_dump(exclude_none=True) for key, badge in data.badges.items()}}, collection=collection))
