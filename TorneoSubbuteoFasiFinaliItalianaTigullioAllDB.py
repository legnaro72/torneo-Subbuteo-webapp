...
HOME_URL = "https://torneo-subbuteo-FF-tigullio-ita-all-db.streamlit.app/"
...
def render_round(df_round, round_idx, modalita_visualizzazione="squadre"):
    ...
    for idx, match in df_temp.iterrows():
        ...
        if modalita_visualizzazione == "completa":
            label_a = f"{squadra_a} ({giocatore_a})" if giocatore_a else squadra_a
            label_b = f"{squadra_b} ({giocatore_b})" if giocatore_b else squadra_b
        elif modalita_visualizzazione == "giocatori":
            label_a = giocatore_a or squadra_a
            label_b = giocatore_b or squadra_b
        else:
            label_a = squadra_a
            label_b = squadra_b
        ...
        if tipo_vista == 'compact':
            ...
            with casa_col:
                st.markdown(
                    ...
                    f"<div class='tigullio-compact-team-name home'>{escape(str(label_a))}</div>",
                    unsafe_allow_html=True
                )
            with gol_casa_col:
            ...
            with osp_col:
                st.markdown(
                    f"<div class='tigullio-compact-team-name away'>{escape(str(label_b))}</div>",
                    unsafe_allow_html=True
                )
            with valida_col:
                result_checkbox(
            ...
            with st.container(border=True):
                ...
                with c1:
                    st.markdown(f"<span class='tigullio-compact-match-marker'></span><div class='tigullio-compact-team-name home'>🏠 {escape(str(label_a))}</div>", unsafe_allow_html=True)
                with c2:
                    result_number_input(f"Gol casa: {label_a}", min_value=0, max_value=20, key=key_gol_a, disabled=is_disabled, label_visibility="collapsed")
                with c3:
                    result_number_input(f"Gol ospite: {label_b}", min_value=0, max_value=20, key=key_gol_b, disabled=is_disabled, label_visibility="collapsed")
                with c4:
                    st.markdown(f"<div class='tigullio-compact-team-name away'>{escape(str(label_b))} 🛫</div>", unsafe_allow_html=True)
        ...
        if 'df_torneo_preliminare' not in st.session_state and 'df_classifica_preliminare' not in st.session_state:
            ...
            if st.session_state.get('giornate_mode'):
                if st.session_state['giornate_mode'] == 'gironi':
                    ...
                    redirect_url = f"https://torneo-subbuteo-tigullio-ita-all-db.streamlit.app/?torneo={urllib.parse.quote(torneo_nome)}"
                    st.markdown(
                        f"""
                        <script>
                    ...
                    if st.session_state.get('show_all_ko_matches', False):
                        ...
                        if st.session_state['rounds_ko']:
                            ...
                            show_save_status()
                            has_write_access = verify_write_access()
                            if st.button("Salva risultati", key="tigullio_ko_save", disabled=not has_write_access):
                                if salva_risultati_ko():
                                    st.rerun()
                            next_label = "Concludi torneo" if len(current_round_df) == 1 else "Genera prossimo round"
                            if st.button(next_label, key="tigullio_ko_advance",
                    ...
                    if st.session_state.get('vincitore_torneo'):
                        st.markdown("---")
                        st.link_button("🏠 Vai alla gestione tornei", "https://farm-tornei-subbuteo-tigullio-all-db.streamlit.app/")
    # Footer leggero
    st.markdown("---")
