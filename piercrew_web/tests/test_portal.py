...
class DomainParity(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        path = Path(__file__).resolve().parents[2] / 'TorneoSubbuteoItalianaPierCrewAllDB.py'
...
class PortalTests(unittest.TestCase):
    ...
    def test_csrf_and_cross_origin_rejected(self):
        self.client.headers.pop('X-PierCrew-Request')
    ...
    def test_live_writes_require_explicit_enable(self):
        ...
        with patch.dict(os.environ,{'PIERCREW_WRITE_ENABLED':'false'}):
    ...
    def test_handoff_allowlist_and_legacy_shape(self):
        ...
        self.assertEqual(doc['collection'],'piercrew_players');self.assertFalse(doc['consumed'])
