...
def main():
    ...
    if not st.session_state.calendario_generato and not st.session_state.get('mostra_form'):
        ...
        with col1:
            st.header("✨ Nuovo Torneo")
            st.write("Inizia da zero e crea un nuovo torneo personalizzato.")
            if st.button("Crea un nuovo torneo", use_container_width=True):
                st.session_state['mostra_form'] = True
                st.rerun()

        with col2:
            st.header("📂 Carica Torneo")
            st.write("Continua un torneo esistente caricando il file CSV.")
            uploaded_file = st.file_uploader("Carica CSV torneo", type=["csv"], label_visibility="collapsed")
    ...
    if st.session_state.get('mostra_form', False):
        ...
        if 'giocatori_scelti' in st.session_state:
