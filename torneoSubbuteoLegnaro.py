...
def main():
    st.title("? Generatore Calendario Torneo Subbuteo")

    num_gironi = st.number_input("?? Numero di gironi", min_value=1, value=2, step=1)

    nomi_input = st.text_area("?? Inserisci i nomi dei giocatori separati da virgola", placeholder="Mario, Luca, Anna, Giulia")

    if st.button("?? Genera Calendario"):
        nomi_giocatori = [n.strip() for n in nomi_input.split(",") if n.strip()]
        ...
        st.download_button(
            label="?? Scarica HTML",
            data=html,
            file_name="torneo_calendario.html",
            mime="text/html"
        )
