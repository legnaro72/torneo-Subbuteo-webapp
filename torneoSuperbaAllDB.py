...
def carica_tornei_da_db(tournementss_collection):
    if tournementss_collection is None:
        return []
    try:
        return list(tournementss_collection.find({}, {"nome_torneo": 1}))
    except Exception as e:
        st.error(f"❌ Errore caricamento tornei: {e}")
        return []

def carica_torneo_da_db(tournementss_collection, tournements_id):
    if tournementss_collection is None:
        return None
    try:
        torneo_data = tournementss_collection.find_one({"_id": ObjectId(tournements_id)})
...
def salva_torneo_su_db(tournementss_collection, df_torneo, nome_torneo):
    if tournementss_collection is None:
        return None
    try:
        ...
        result = tournementss_collection.insert_one(data)
        return result.inserted_id
    except Exception as e:
        st.error(f"❌ Errore salvataggio torneo: {e}")
        return None

def aggiorna_torneo_su_db(tournementss_collection, tournements_id, df_torneo):
    if tournementss_collection is None:
        return False
    try:
        df_torneo_pulito = df_torneo.where(pd.notna(df_torneo), None)
        tournementss_collection.update_one(
            {"_id": ObjectId(tournements_id)},
...
def salva_risultati_giornata(tournementss_collection, girone_sel, giornata_sel):
    ...
    if 'tournements_id' in st.session_state:
        aggiorna_torneo_su_db(tournementss_collection, st.session_state['tournements_id'], df)
...
def main():
    ...
    tournementss_collection = init_mongo_connection(st.secrets["MONGO_URI_TOURNEMENTS"], "subbuteo_tournement", "superba_tournement", show_ok=False)
    ...
    if players_collection is None and tournementss_collection is None:
    ...
    if st.session_state.get('calendario_generato', False):
        ...
        if st.session_state['filtro_attivo'] == 'Nessuno':
            ...
            st.button("💾 Salva Risultati Giornata", on_click=salva_risultati_giornata, args=(tournementss_collection, st.session_state['girone_sel'], st.session_state['giornata_sel']))
        ...
        with col1:
            tornei_disponibili = carica_tornei_da_db(tournementss_collection)
            if tornei_disponibili:
                ...
                if st.button("Carica Torneo Selezionato"):
                    st.session_state['tournements_id'] = tornei_map[nome_sel]
                    st.session_state['nome_torneo'] = nome_sel
                    torneo_data = carica_torneo_da_db(tournementss_collection, st.session_state['tournements_id'])
                    if torneo_data and 'calendario' in torneo_data:
        ...
        if st.session_state.get('mostra_form_creazione', False):
            ...
            if st.session_state.get('mostra_gironi', False):
                ...
                if st.button("Genera Calendario"):
                    ...
                    tid = salva_torneo_su_db(tournementss_collection, df_torneo, st.session_state['nome_torneo'])
                    if tid:
                        st.session_state['df_torneo'] = df_torneo
                        st.session_state['tournements_id'] = str(tid)
                        st.session_state['calendario_generato'] = True
