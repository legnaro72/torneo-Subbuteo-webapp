> Clone Tigullio generato da ClonaMigrazione.py. Configurare un progetto Vercel e i segreti specifici del club prima della pubblicazione.

# Portale Tigullio

Portale React/Vite con API FastAPI per il **torneo all’italiana**, le **fasi finali**, lo **svizzero** e la **gestione club**. Usa gli utenti e i documenti MongoDB del mondo Tigullio. Le app classiche rimangono disponibili tramite token `auth_handoff` monouso, già supportato da `shared/auth/session_manager.py`.
...
Da `tigullio_web/`, in PowerShell:

```powershell
py -3.12 -m venv .venv
.\.venv\Scripts\python.exe -m pip install -r requirements-dev.txt
npm install
$env:TIGULLIO_DEMO='true'
$env:TIGULLIO_APP_ORIGIN='http://127.0.0.1:5173'
...
Aprire `http://127.0.0.1:5173`. Credenziali **solo demo**: `Andrea` / `tigullio-demo`. La demo è interamente in memoria e non usa l’URI MongoDB. Al riavvio dell’API i dati demo ripartono dall’inizio.
...
| Variabile | Uso |
|---|---|
| `MONGO_URI` | Cluster dei giocatori (`giocatori_subbuteo.tigullio_players`) |
| `MONGO_URI_AUTH` | Cluster di utenti, password di sistema e sessioni. Se assente, usa `MONGO_URI` |
| `MONGO_URI_TOURNEMENTS` | Cluster `TorneiSubbuteo.Tigullio` e `TorneiSubbuteo.TigullioSvizzero`. Se assente, usa `MONGO_URI` |
| `TIGULLIO_APP_ORIGIN` | Origine esatta del frontend, ad es. `https://nome.vercel.app` |
| `TIGULLIO_WRITE_ENABLED` | `true` in produzione per consentire salvataggi avviati da utenti autenticati con ruolo `A` o `W`; per impostazione predefinita i comandi sui tornei sono bloccati |
| `TIGULLIO_DEMO` | `true` solo per la demo locale; su Vercel viene rifiutato |
...
Questo clone non è ancora pubblicato; usare variabili d'ambiente e progetto Vercel separati. Nessun dato MongoDB viene copiato dallo script.
