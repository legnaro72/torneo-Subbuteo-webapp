...
async def guard(request: Request, call_next):
    if request.method not in ('GET', 'HEAD', 'OPTIONS'):
        expected = os.getenv('TIGULLIO_APP_ORIGIN', str(request.base_url).rstrip('/'))
...
@app.get('/api/config')
def config():
    demo = os.getenv('TIGULLIO_DEMO', '').lower() == 'true' and not os.getenv('VERCEL')
    return {'demo': demo, 'writes_enabled': demo or os.getenv('TIGULLIO_WRITE_ENABLED', '').lower() == 'true'}
...
def handoff(destination: str, user=Depends(current_user), store: Store = Depends(store_dep)):
    ...
    store.handoffs.insert_one({'token_hash': hash_token(token), 'user_id': user['id'], 'username': user['username'],
                               'role': user['role'], 'collection': 'tigullio_players', 'created_at': now,
...
def create(data: CreateTournament, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    if existing:
        if existing.get('_tigullio_create_hash') != payload_hash:
    ...
    doc = {'_id': new_id, 'nome_torneo': data.name, 'calendario': calendar.to_dict('records'), 'data_creazione': datetime.utcnow(),
           'data_modifica': datetime.utcnow(), '_tigullio_revision': 0, '_tigullio_create_hash': payload_hash,
           '_tigullio_return_matches': data.return_matches}
    if data.badges:
        doc['_tigullio_badges'] = {key: badge.model_dump(exclude_none=True) for key, badge in data.badges.items()}
...
def withdrawal(tournament_id: str, data: Withdrawal, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    retired = set(doc.get('_tigullio_withdrawals', [])) | set(data.teams)
    rows = [dict(row) for row in doc['calendario']]
    for row in rows:
    ...
    return view(save(store, doc, data.version, {'calendario': rows, '_tigullio_withdrawals': sorted(retired)}))
...
def complete(tournament_id: str, data: Complete, user=Depends(writer), store: Store = Depends(store_dep)):
    doc = load(store, tournament_id)
    require_tournament_write(user, doc['nome_torneo'])
    if doc.get('_tigullio_source_id') or doc['nome_torneo'].startswith('completato_'):
...
def export_csv(tournament_id: str, user=Depends(current_user), store: Store = Depends(store_dep)):
    ...
    return Response(output.getvalue().encode('utf-8-sig'), media_type='text/csv', headers={'Content-Disposition': 'attachment; filename="calendario-tigullio.csv"'})
...
def save_badges(kind: str, tournament_id: str, data: SaveBadges, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    return render(save(store, doc, data.version, {'_tigullio_badges': {key: badge.model_dump(exclude_none=True) for key, badge in data.badges.items()}}, collection=collection))
