...
# Rileva se siamo nel club Tigullio per mostrare il Palmares (solo per Tigullio)
IS_SUPERBA = "tigullio" in collection_players.name.lower()
...
# ==============================================================================
# PDF EXPORT — La Gazzetta della Tigullio (Composizione Club)
# ==============================================================================

class GazzettaClubPDF(FPDF):
    """PDF con stile 'La Gazzetta della Tigullio' per la composizione del club."""
...
def genera_pdf_club(df_giocatori, df_tornei_ita, df_tornei_svizzeri, is_tigullio=True):
...
if st.session_state.edit_index is None and st.session_state.confirm_delete["type"] is None:
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("📰 La Gazzetta della Tigullio")
    ...
    if 'pdf_club_pronto' in st.session_state and st.session_state['pdf_club_pronto'] is not None:
        st.sidebar.download_button(
            label="📥 Scarica PDF Club Tigullio",
            data=st.session_state['pdf_club_pronto'],
            file_name=f"Gazzetta_Club_Tigullio_{datetime.now().strftime('%d%m%Y')}.pdf",
