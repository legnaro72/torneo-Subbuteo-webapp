...
def esporta_pdf(df_torneo, df_classifica):
    ...
    for girone in gironi:
        ...
        if pdf.get_y() + 8 + margin_bottom > page_height: pdf.add_page()
        ...
        for g in giornate:
            ...
            if pdf.get_y() + needed_space > page_height: pdf.add_page()
        ...
        if not df_classifica.empty:
            if pdf.get_y() + 40 + margin_bottom > page_height: pdf.add_page()
...
def main():
    ...
    #             n_giornate = df['Giornata'].max()
    #             df_girone = df_girone[df_girone['Giornata'] <= n_giornate / 2] if filtro_tipo_g == "Andata" else df_girone[df_girone['Giornata'] >
